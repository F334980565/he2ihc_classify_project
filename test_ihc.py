...
from data import LabeledDataset, CompleteDataset
from model import HE_classifier, IHC_classifier
...
def test_HE(model, state_dict_path, save_path = None, device='cuda:0'):
    if not save_path == None:
        os.makedirs(os.path.join(save_path, 'csv'), exist_ok=True)
        os.makedirs(os.path.join(save_path, 'img'), exist_ok=True)
...
test_HE(model, 
        state_dict_path = '/root/projects/wu/classify_project/checkpoints/HEclassifier/HEclassifer_10epoch.pth', 
        save_path = '/root/projects/wu/classify_project/probs_save/HE_probs_1')